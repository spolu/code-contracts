...
/// @cc [owner:spolu,label:product] rust-state-fixture
/// Represents a separate Rust declaration for whole-file listing.
pub enum State {
    #[allow(dead_code)]
    /// @cc [owner:spolu,label:product] rust-paid-variant-fixture
    /// Defines a contract on an attributed enum variant.
    Paid,
}

/// @cc [owner:spolu,label:product] rust-payer-fixture
/// Defines a trait whose members can carry contracts.
pub trait Payer {
    /// @cc [owner:spolu,label:product] rust-payer-signature-fixture
    /// Defines a contract on a trait method signature.
    fn pay(&mut self, amount: i64) -> i64;
}

/// @cc [owner:spolu,label:product] rust-default-amount-fixture
